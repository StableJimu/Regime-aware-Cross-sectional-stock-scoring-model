...
def chunked_read_panel_csv(panel_path: Path, tickers: List[str], chunksize: int = 1_000_000) -> pd.DataFrame:
...
def batch_items(iterable: List[str], n: int) -> Iterable[List[str]]:
    for i in range(0, len(iterable), n):
        yield iterable[i : i + n]


def existing_batch_numbers(out_dir: Path) -> set[int]:
    nums: set[int] = set()
...
def compute_factor_batches(cfg: dict, logger, force: bool = False) -> Path:
    ...
    existing = existing_batch_numbers(out_dir)

    for i, tick_batch in enumerate(batch_items(tickers, batch_size), start=1):
        out_path = out_dir / f"factors_raw_batch_{i:03d}.parquet"
        if not force and i in existing and out_path.exists():
        ...
        raw_prices = chunked_read_panel_csv(panel_path, tick_batch)
        if raw_prices.empty:
